const keep_alive = require ('./keep_alive.js') // Intenta mantener la conexiÃ³n cada 60 // Import necessary modules from the "highrise.sdk" package


const { Highrise, GatewayIntentBits, WebApi } = require("highrise.sdk");

// Import settings for the bot
const { authentication } = require("./config/config.js");
const { handleReady, handleChat, handleWhisper, handleDirectMessages, handlePlayerJoin, handlePlayerLeave, handlePlayerEmotes, handlePlayerReactions, handlePlayerTips, handlePlayerMovements, handleVoiceChat, handleErrors } = require("./handlers/events.js");
const { sendPeriodicMessage } = require("./utils/utils.js");
...
bot.on('playerJoin', (user, react) => {
  handlePlayerJoin(bot, user, react);
});
